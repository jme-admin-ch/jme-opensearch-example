...
The transit-document flow also demonstrates collection fields. Its generated data record contains a top-level
`List<String>` and a `List` of `nested` customs-check records whose children include collections at multiple nesting
levels. The end-to-end test verifies that these values survive resource serialization, indexing, and typed search-result
deserialization.
